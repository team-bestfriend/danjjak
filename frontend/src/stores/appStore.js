...
const ERROR_MESSAGES = {
  ...
  SOURCE_ACCOUNT_NOT_FOUND:
    "선택한 출금 계좌를 찾을 수 없습니다. 계좌를 다시 선택해 주세요.",
  RECIPIENT_ACCOUNT_NOT_FOUND:
    "받는 계좌를 찾을 수 없습니다. 받는 사람을 다시 선택해 주세요.",
  ...
  SESSION_REQUIRED:
    "로그인이 만료되었습니다. 카카오 로그인 후 다시 시도해 주세요.",
  ...
  INVALID_RESPONSE:
    "서버 응답을 확인할 수 없습니다. 거래내역에서 처리 결과를 확인해 주세요.",
};
...
function invalidResponse() {
  return new ApiError("INVALID_RESPONSE", ERROR_MESSAGES.INVALID_RESPONSE, 502);
}
...
function nicknameStorageKey(user) {
  const userId = user?.userId ?? user?.id ?? user?.kakaoUserId ?? "current";

  return `danjjak:nickname:${userId}`;
}

function readSavedUserName(user) {
  if (typeof window === "undefined") return "";

  return localStorage.getItem(nicknameStorageKey(user))?.trim() ?? "";
}
...
export const useAppStore = defineStore("app", () => {
  ...
  const defaultOwnedAccount = computed(
    () =>
      ownedAccounts.value.find((account) => account.primary) ??
      ownedAccounts.value[0] ??
      null,
  );

  const selectedSourceAccount = computed(
    () =>
      ownedAccounts.value.find(
        (account) => account.accountId === selectedSourceAccountId.value,
      ) ?? null,
  );
  ...
  function applyCurrentUser(user) {
    const savedUserName = readSavedUserName(user);
    const resolvedUserName = savedUserName || user.name || "순자";
  ...
  function saveUserName(name) {
    ...
    if (!normalizedName || normalizedName.length > 10) {
      throw new Error("이름을 1자 이상 10자 이하로 입력해 주세요.");
    }
    ...
    if (typeof window !== "undefined") {
      localStorage.setItem(
        nicknameStorageKey(currentUser.value),
        normalizedName,
      );
    }
  ...
  function setPatterns(newPatterns) {
    patterns.value =
      typeof newPatterns === "function"
        ? newPatterns(patterns.value)
        : newPatterns;
  }
  ...
  async function loadPatternTemplates(force = false) {
    if (patternTemplates.value.length > 0 && !force)
    ...
    try {
      ...
      patternError.value = toMessage(
        error,
        "등록 가능한 금융 업무를 불러오지 못했습니다.",
      );
      throw error;
    } finally {
      patternLoading.value = false;
  ...
  async function persistPatternOrder(nextPatterns) {
    const saved = await patternApi.reorderPatterns(
  ...
  async function reorder(sourceNum, targetNum) {
    ...
    try {
      ...
      showToast(
        `${dragged?.label ?? "패턴"}이(가) ${targetNum}번으로 변경됐어요.`,
        "되돌리기",
        () => {
  ...
  function mapRegisteredPerson(person) {
    return {
      ...
      name: person.name,
      emoji:
        person.relationship === "아들"
          ? "👨"
          : person.relationship === "딸"
            ? "👩"
            : "👤",
  ...
  function loadFinancialData(force = false) {
    ...
    pendingFinancialDataLoad = (async () => {
      ...
      try {
        ...
        if (
          ownedAccounts.value.length > 0 &&
          !ownedAccounts.value.some((account) => account.primary)
        ) {
          financeWarning.value =
            "기본 계좌가 지정되지 않아 첫 번째 본인 계좌를 선택했어요.";
        }
        people.value = personRows.map(mapRegisteredPerson);
        accountsByPerson.value = Object.fromEntries(
        ...
        if (
          !ownedAccounts.value.some(
            (account) => account.accountId === selectedInquiryAccountId.value,
          )
        ) {
          selectedInquiryAccountId.value =
        ...
        financeError.value = toMessage(
          error,
          "계좌와 등록 인물을 불러오지 못했습니다.",
        );
        ownedAccounts.value = [];
        people.value = [];
        accountsByPerson.value = {};
  ...
  function loadMockAccountImportOptions(force = false) {
    ...
    if (mockAccountImportOptions.value.length > 0 && !force)
    ...
    pendingAccountImportOptionsLoad = (async () => {
      ...
      try {
        ...
        mockAccountImportOptions.value = [];
        accountImportError.value = toMessage(
          error,
          "계좌를 불러오지 못했습니다.",
        );
        return false;
  ...
  async function saveRecipientAccount(
    ...
    const saved = accountId
      ? await accountApi.updateRecipientAccount(
          registeredPersonId,
          accountId,
          payload,
        )
  ...
  async function loadInquiry(accountId, category = null) {
    ...
    try {
      ...
      if (requestSequence === inquiryRequestSequence)
        inquiryLoading.value = false;
    }
  }
  ...
  async function startPatternExecution(pattern) {
    ...
    try {
      ...
      const firstStep = [...detail.steps].sort(
      ...
      patternExecutionError.value = toMessage(
        error,
        "금융 패턴을 시작하지 못했습니다.",
      );
      throw error;
  ...
  async function closeCurrentStepVisit() {
    if (
      !executionLoggingEnabled.value ||
      !patternExecutionId.value ||
      !currentStepVisit.value
    )
      return;
  ...
  function syncPatternStep(screenCode) {
    ...
    patternStepSync = patternStepSync
      .then(async () => {
        const step = activePatternDetail.value?.steps?.find(
        ...
        if (
          !step &&
          currentStepVisit.value &&
          !EXPECTED_PATTERN_BRANCH_ROUTES.has(screenCode)
        ) {
        ...
        if (
          ...
          return;
        currentStepVisit.value = await patternApi.startVisit(
          patternExecutionId.value,
          step.stepId,
        );
      })
  ...
  function recordPatternAction(action) {
    ...
    patternStepSync = patternStepSync
      .then(() => patternApi.updateVisit(executionId, visitId, payload))
      .catch((error) => {
        patternExecutionError.value = toMessage(
          error,
          "단계 행동 기록을 저장하지 못했습니다.",
        );
      });
  }
  ...
  async function finishPatternExecution(status) {
    ...
    try {
      ...
      if (executionLoggingEnabled.value && patternExecutionId.value) {
        return await patternApi.finishExecution(
          patternExecutionId.value,
          status,
        );
      }
  ...
  function selectPerson(personId) {
    ...
    selectedAccountMasked.value =
      accounts.length === 1 ? accounts[0].masked : null;
  }
  ...
  function createTransferPayload(pin) {
    ...
    if (patternExecutionId.value)
      payload.patternExecutionId = patternExecutionId.value;
    if (isNewAccountFlow.value) payload.directRecipient = directRecipient.value;
    else
      payload.registeredRecipientAccountId = selectedRecipientAccountId.value;
    return payload;
  }
    ...
    try {
      ...
      if (response.status === "REQUIRES_REVIEW") {
        ...
        transferResult.value = {
          ...response,
          recipientName: isNewAccountFlow.value
            ? directRecipient.value?.name
  ...
  async function sendGuardianNotification() {
    if (
      !anomaly.value?.anomalyEventId ||
      anomaly.value.riskLevel !== "HIGH" ||
  ...
  async function refreshAfterTransfer() {
    postTransferSyncError.value = "";
    await loadFinancialData(true);
    if (selectedSourceAccountId.value)
      await loadInquiry(selectedSourceAccountId.value);
    const syncErrors = [financeError.value, inquiryError.value].filter(Boolean);
    if (syncErrors.length > 0) {
      postTransferSyncError.value =
        "송금은 완료됐지만 최신 잔액과 거래내역을 다시 불러오지 못했습니다.";
    }
  }
