<template>
  <div
    class="flex h-full flex-col items-center justify-center bg-[#FAFAF8] px-7 text-center"
  >
    <div v-if="processing" class="flex flex-col items-center" role="status">
      <div
        class="h-16 w-16 animate-spin rounded-full border-8 border-[#FFF3CC] border-t-[#FFBC00]"
      />

      <h1 class="mt-7 text-[26px] font-bold text-[#111827]">
        로그인을 확인하고 있어요
      </h1>
    ...
    <div v-else>
      <div class="text-[64px]" aria-hidden="true">
        {{ cancelled ? "↩️" : "⚠️" }}
      </div>
      ...
      <button
        type="button"
        class="mt-3 h-[52px] w-full text-[16px] text-[#6B7280]"
        @click="router.replace({ name: 'onboarding' })"
      >
        처음 화면으로
      </button>
    </div>
...
const cancelled = computed(() => {
  return route.query.status === "cancelled";
});

const ERROR_MESSAGES = {
  OAUTH_STATE_INVALID:
    "로그인 확인 정보가 만료되었습니다. 처음부터 다시 시도해 주세요.",
...
onMounted(async () => {
  ...
  /*
   * 개인정보 동의 화면을 거친 최초 시작 흐름에서만
   * 닉네임 설정 화면을 표시합니다.
   *
   * 다시 이용하기는 개인정보 동의 화면을 거치지 않으므로
   * 이 값이 없고 닉네임 설정도 건너뜁니다.
   */
  const firstStartFlow =
    sessionStorage.getItem("danjjakPrivacyConsent") !== null;

  if (firstStartFlow) {
    await router.replace({ name: "nickname-setup" });
    return;
  }
