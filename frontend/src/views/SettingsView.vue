<template>
  <div className="flex flex-col h-full" style="background: #fafaf8">
    <SafeArea />
    <div
      className="bg-white px-5 border-b border-[#EEEEED] flex-shrink-0"
    ...
    <div className="flex-1 overflow-y-auto px-4 pt-4 pb-4 space-y-5">
      ...
      <Card className="p-5">
        <div className="flex items-center gap-4">
          <div
            className="w-16 h-16 rounded-full bg-white border-2 border-[#FFE08A] overflow-hidden flex items-center justify-center flex-shrink-0"
          >
            <img
              :src="oldAgeWoman"
              alt=""
          ...
          <div className="flex-1">
            <p className="font-bold text-[#111827]" style="font-size: 23px">
              {{ store.userName }}
            </p>
          </div>
        </div>
      ...
      <div>
        <p
          className="font-bold text-[#9CA3AF] uppercase tracking-wide mb-3 px-1"
        ...
        <Card className="p-5 space-y-5">
          <div>
            ...
            />
            <p
              class="mt-3 rounded-[14px] bg-[#FFFBEB] p-4 text-[#374151]"
              :style="{
                fontSize: `calc(18px * ${{ small: 0.95, normal: 1, large: 1.15 }[fontSize]})`,
              }"
            >
              이 크기로 안내를 읽어요.<br />버튼 크기는 그대로 유지해요.
            </p>
          </div>
        </Card>
      </div>
        ...
        <Card className="p-5 space-y-5">
          ...
          <div className="border-t border-[#F3F4F6] pt-5">
            ...
            <p className="text-[#9CA3AF] mt-2 px-1" style="font-size: 13px">
              {{
                voiceMode === "tts"
                  ? "현재 안내 문구를 AI 음성으로 읽어요."
                  : "저장된 가족 녹음이 없거나 재생되지 않으면 같은 문구를 AI 음성으로 읽어요."
              }}
              패턴에서 따로 선택한 음성은 유지돼요.
            </p>
          </div>
          <button
            type="button"
            ...
            @click="saveSettings"
          >
            {{ settingsSaving ? "저장 중…" : "화면·음성 설정 저장" }}
          </button>
          <p v-if="settingsError" className="text-[#B91C1C]" role="alert">
            {{ settingsError }}
          </p>
          <p v-if="settingsSaved" className="text-[#166534]" role="status">
            설정을 저장했어요.
          </p>
        </Card>
      </div>
      ...
      >
        <span className="font-semibold text-[#111827]" style="font-size: 18px"
          >선택 동의 관리</span
        >
        <span className="mt-1 block text-[#6B7280]" style="font-size: 14px"
          >이용 기록과 카카오 나에게 보내기 동의를 변경해요.</span
        >
      </button>
      ...
      <div>
        ...
        <Card className="p-5 space-y-3">
          <div>
            ...
            <p className="text-[#6B7280] mb-3" style="font-size: 15px">
              이상 거래 화면의 보호자 전화 연결에 사용해요.
            </p>
            <p v-if="store.supportLoading" className="text-[#6B7280] mb-3">
              보호자 번호를 불러오고 있어요…
            </p>
            <p
              v-else-if="!store.supportError && !guardianPhone"
              className="text-[#6B7280] mb-3"
            >
              등록된 보호자 번호가 없어요. 아래에 번호를 입력해 주세요.
            </p>
            <div className="flex gap-2 w-full">
              ...
              >
                {{ store.guardianSaving ? "저장 중…" : "저장" }}
              </button>
            </div>
            <p
              v-if="guardianError || store.supportError"
              className="text-[#B91C1C] mt-2"
              role="alert"
            >
              {{ guardianError || store.supportError }}
            </p>
            <p v-if="guardianSaved" className="text-[#166534] mt-2">
              저장한 번호를 보호자 전화 연결에 반영했어요.
            </p>
          </div>
        </Card>
      </div>
      ...
      <div>
        ...
        <Card className="overflow-hidden">
          <button
            ...
            @click="
              store.navigate('account-import', { query: { manage: '1' } })
            "
          >
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-[#9CA3AF]" style="font-size: 14px">
                  본인 계좌 확인·추가
                </p>
              </div>
            </div>
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-[#9CA3AF]" style="font-size: 14px">
                  가족 {{ store.people.length }}명 · 송금 계좌 관리
                </p>
              </div>
            </div>
            <Ic name="ChevR" />
      ...
      <div>
        ...
        <Card className="overflow-hidden">
          <button
            ...
            :class="[
              'w-full flex items-center justify-between px-5 py-4 text-left active:bg-[#F9FAFB] disabled:cursor-not-allowed disabled:opacity-60',
...
const {
  ...
  {
    speed: () => guideSpeed.value.toUpperCase(),
...
watch([fontSize, guideSpeed, voiceMode], () => {
...
async function saveSettings() {
  ...
  try {
    ...
    settingsError.value =
      error?.message ?? "설정을 저장하지 못했습니다. 다시 시도해 주세요.";
  } finally {
...
async function saveGuardian() {
  ...
  if (!/^[0-9-]{8,20}$/.test(guardianPhone.value)) {
    guardianError.value =
      "전화번호는 숫자와 하이픈을 포함해 8~20자로 입력해 주세요.";
    return;
  }
  try {
