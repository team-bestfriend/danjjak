  ...
  <div class="flex h-full flex-col bg-[#FAFAF8]">
    ...
    <main class="flex-1 overflow-y-auto px-5 pb-5 pt-8">
      ...
      <p class="mt-3 text-[16px] leading-relaxed text-[#6B7280]">
        두 항목은 모두 선택 사항입니다. 동의하지 않아도 단짝의 기본 기능을
        이용할 수 있어요.
      </p>
      ...
      <p
        v-if="errorMessage"
        class="mt-5 rounded-[14px] bg-[#FEF2F2] p-4 text-[15px] text-[#B91C1C]"
        role="alert"
      >
        {{ errorMessage }}
      </p>
...
const usageLogAgreed = ref(
  Boolean(store.currentUser?.consents?.usageLogAgreed),
);
const guardianShareAgreed = ref(
...
async function save() {
  ...
  try {
    ...
    errorMessage.value =
      error?.message ?? "동의 내용을 저장하지 못했습니다. 다시 시도해 주세요.";
  } finally {
    saving.value = false;
  }
}
