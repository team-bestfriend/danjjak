<template>
  <div class="flex h-full flex-col bg-[#FAFAF8]">
    ...
    <main class="min-h-0 flex-1 space-y-5 overflow-y-auto px-5 py-6">
      ...
      <p v-if="loading" role="status">문구 제안을 불러오고 있어요…</p>
      ...
      <template v-else-if="suggestion">
        ...
        <Card class="space-y-3 p-5">
          ...
          <p class="break-words text-[19px] leading-relaxed">{{ suggestion.currentText }}</p>
        </Card>
        <Card class="space-y-3 border-2 border-[#FFBC00] bg-[#FFFBEB] p-5">
          <h2 class="text-[17px] font-bold text-[#92650A]">더 쉽게 설명한 제안</h2>
          <p class="break-words text-[19px] leading-relaxed">{{ suggestion.suggestedText }}</p>
        </Card>
        ...
        <Btn v-if="stale" variant="secondary" :disabled="saving" @click="load">바뀐 문구 다시 비교하기</Btn>
        <Btn :disabled="saving || applied || sameText || stale" @click="apply">{{ saving ? '적용 중…' : applied ? '적용 완료' : sameText ? '이미 같은 문구예요' : '이 제안 문구 적용하기' }}</Btn>
        ...
        <Btn variant="secondary" :disabled="saving" @click="edit(false)">직접 문구와 음성 편집하기</Btn>
      </template>
      <template v-else>
...
async function apply() {
  ...
  try {
    const saved = await instructionSuggestionApi.apply(target.patternId, target.stepId, target.currentText);
