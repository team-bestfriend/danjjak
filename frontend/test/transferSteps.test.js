...
test('저장된 수취 계좌가 유효한 단축번호 송금은 4단계를 연속으로 사용한다', () => {
  const steps = SHORTCUT_TRANSFER_FLOW.map((screen) => transferStepBar(screen, true));
  const orders = steps.map((step) => step.current);
