...
| FR-006 | 사람·계좌 관리 | 사람 추가·수정·삭제, 기존 사람의 계좌 추가·수정·삭제 |
...
형식 검증만 수행하며 실존 계좌·예금주 확인은 범위에서 제외한다. 단축번호에 연결된 사람과 받는 계좌는 삭제하지 않으며 기존 연결도 자동 변경하지 않는다.
