...
| 구분 | 확인 항목 |
| --- | --- |
| 정상 | 소개 3개 메시지 → 카카오 → 같은 모의 사용자 → 닉네임 → 두 동의 → ‘계좌를 연동할게요’ → ‘계좌 불러오기’ → 노란 점 세 개의 2초 로딩 → 보호자 설정 → 계좌 미준비 시 모의 계좌 선택·추가 → 홈 |
...
| 조회 결과 안내 | 연금 입금·관리비·공과금 최신 한 건의 날짜·상대방·금액·입출금 방향이 화면과 TTS에서 일치. 가족 음성 설정에서도 결과는 TTS, 시작 전 설정은 유지. 계좌 변경 중 이전 안내 중단, 빈 결과·실패·계좌 없음 구분 |
...
| 단계 UI | 직접 송금·송금 패턴 모두 보낼 계좌를 누르면 받는 사람 단계로 이동. 뒤로 돌아오면 선택만 유지하고 자동 이동하지 않음 |
| 선택 유지 | 받는 사람·계좌는 노란색 라디오로 표시. 뒤로 이동·같은 사람 재선택 시 계좌 유지, 다른 사람 선택 시 그 사람의 계좌로 전환 |
...
| 화면 | 쉬운 두 사유·높은 주의·받는 분과 금액·‘가족에게 알림 보내기’. 화면·음성에서 기술 용어와 시연·본인 수신 설명 제외 |
| 안내 | 단짝이 말풍선·음성은 전화·문자의 송금 요구·재촉에 주의하고 가족과 확인하도록 안내. 실제 두 사유는 ‘이번 송금에서 확인할 점’에 표시하며 사기 확정 문구 없음 |
| 결과 | 실제 성공 / 자격 없음 모의 / 실제 실패 후 모의 |
| 결과 안내 | 실제 성공은 ‘카카오톡 알림을 보냈어요.’와 보낸 시간. 두 모의 결과는 ‘카카오톡 알림을 보내지 못했어요.’와 가족 전화 안내. 내부 상세 문구 노출 없음 |
...
| 편집 이탈 | 업무 등록·패턴/음성 수정 중 화면/브라우저 뒤로 가기는 내부 모달로 확인. 계속 작성·Escape·배경 클릭은 초안 유지, 확인은 이동. 중복 확인 없음, 저장 완료 후 바로 이동 |
...
| 홈 음성 선택 | 듣기·확인 중·실패 문구는 단짝이 말풍선에만 표시. X로 닫아도 인식 유지, 새 상태에서 안내 재표시. 전체 폭 배경 없이 말풍선·아이콘만 겹치며 하단 메뉴 조작 가능. 실행 전 확인·취소·시작 흐름 유지 |
| 송금 UI | 받는 사람·계좌 문구와 기본 음성 일치, 확대 키패드는 428×926 및 320×568·큰 글씨에서 겹침·잘림 없이 조작 가능 |
