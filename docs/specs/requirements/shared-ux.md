...
| 계좌 선택 | 송금의 받는 사람·계좌는 노란색 라디오 + 흰색 내부 점으로 선택 표시, 그 외 계좌 선택은 체크 + ‘선택됨’ |
| 입력·버튼 | 안전 영역·주소창·키보드·큰 글씨에도 겹침·잘림 없음 |

## 편집 이탈 확인

- 새 금융 업무·패턴·설명/단계 음성 편집의 미저장 이탈은 브라우저 확인창 대신 중앙 모달로 안내한다.
- 어두운 배경 위 흰색 둥근 카드에 제목·설명·두 버튼을 표시한다. 기본 문구는 ‘작성을 그만둘까요?’ / ‘저장하지 않은 변경사항은 사라져요.’ / ‘계속 작성’·‘나가기’.
- 계속 작성·Escape·배경 클릭은 초안을 유지한다. 확인 후에만 요청한 이동이나 초안 버리기를 진행한다.
- 모달 밖 조작과 초점 이동을 막고, 닫으면 이전 조작 위치로 초점을 돌린다. 변경 없음·저장 완료는 바로 이동하고 저장 중에는 이탈하지 않는다.
...
| 내 계좌 | 제목 ‘보낼 계좌 선택’ / ‘어느 계좌에서 보낼까요?’ | 누르면 받는 사람 단계로 이동 |
| 내 계좌 설명 | ‘돈이 빠져나갈 내 계좌를 선택해 주세요.’ | — |
| 받는 사람 | ‘받는 사람을 선택해 주세요.’ / ‘등록 계좌 2개’ | 누르면 받는 계좌 단계로 이동 |
| 받는 계좌 | ‘받는 계좌를 선택해 주세요.’ | 누르면 금액 단계로 이동 |
| 금액 | ‘얼마를 보낼까요?’ / 숫자·한글 | 높이 72px 이상 키패드, 다음 |
