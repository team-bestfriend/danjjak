...
아래 상태는 결정·명세 상태이며 코드 구현·검증 완료를 뜻하지 않는다. 구현·검증 체크리스트와 진행 상황은 Notion에서 별도로 관리한다.

| 상태 | 의미 |
| --- | --- |
| 확정 | 사용자 답변으로 결정 |
| 명세 반영 | 목표 명세에 반영, 구현 완료 의미 아님 |
| 설계안 | 화면·표현의 기본 제안 |
| 선택 검토 | 검증 후 채택 여부 결정 |
...
| 카톡 | ‘가족에게 알림 보내기’ 표시, 사용자 화면에서 시연·본인 수신 설명 제외. [FDS](fds-guardian.md) 참고 |
...
| ID | 의견 → 반영 | 관련 항목 | 상태 |
| --- | --- | --- | --- |
| CH-01 | 분석 로그형 문구 → 쉬운 사실·행동 안내 | FR-048–050, [분석](usage-analysis.md) | 명세 반영 |
| CH-02 | 많이 쓴 패턴 위로 → 완료 횟수 내림차순 | FR-048 | 명세 반영 |
| CH-03 | TTS 톤 → 차분·명료한 한국어 | FR-022, [음성](guidance-voice.md) | 목표 명세 반영·음색 선택 |
| CH-04 | iPhone 12 Pro Max → 428×926·큰 글씨·키보드 | NFR-012, [UX](shared-ux.md) | 명세 반영 |
| CH-05 | 번호만 말하기 → 번호 매칭·기존 확인창 | FR-027·016 | 확정 |
| CH-06 | 음성 설명 축소 → 짧은 예시·도움말 | FR-060 | 명세 반영 |
| CH-07 | 업무 내용 크게 → 홈·목록·확인창 공통 | UX-011, [패턴](shortcuts-patterns.md) | 명세 반영 |
| CH-08 | TTS 호칭 → AI 음성, 단짝이 별칭 보류 | D-04 | 설계안 |
| CH-09 | 설명·단계 음성 편집 → 패턴 수정에 통합 | FR-054–056 | 명세 반영 |
| CH-10 | 폰트·아이콘 → 일관된 체계·이모티콘 정리 | D-04 | 원칙 명세 반영·자산 선택 |
| CH-11 | 분석 목적 → 업무·단계 확인 후 안내 개선 | FR-048–052, D-02 | 설계안 |
| CH-12 | 이름 아래 ‘단짝 시연 사용자’ 삭제 | FR-003, [설정](auth-settings.md) | 명세 반영 |
| CH-13 | 생성/수정 시 미리 생성 → 저장 후 준비·갱신 | FR-061, D-03 | 선택 검토 |
| CH-14 | 상·하단 버튼 확대 → 최소 48×48 CSS px | UX-011 | 명세 반영 |
| CH-15 | 금액 읽기 → 쉼표·원·한글 병기 | FR-059, [송금](mock-transfer.md) | 명세 반영 |
| CH-16 | 카톡 문구 → 가족 알림 표시·쉬운 결과 안내, 사용자 화면에서 시연 설명 제외 | FR-042, [FDS](fds-guardian.md) | 확정 |
| CH-17 | 온보딩 → 단축번호·가족 안내·이상 거래 | FR-001 | 명세 반영 |
| CH-18 | 상시 음성/대본 축소 → 상태·다시 듣기·펼치기 | FR-021, D-01 | 설계안 |
| CH-19 | 직접 송금 분리 → 입력별 화면 | FR-029, D-05 | 분리 명세 반영·순서 설계안 |
| CH-20 | 쉬운 용어·단계명·‘‹ 뒤로’ | UX-003·007 | 명세 반영 |
| CH-21 | 확인·인증·완료 → 금액 CTA·본인 확인·구체적 결과 | FR-030–031 | 명세 반영 |
| CH-22 | 경고 문구 → 쉬운 제목·실제 고액/반복 사유 | FR-039 | 명세 반영 |
| CH-23 | 은행 로고 → 로고+이름·대체 표시 | D-04 | 원칙 명세 반영·자산 선택 |
| CH-24 | 계좌 화살표 → 체크·선택됨 | UX-011, [계좌](people-accounts.md) | 명세 반영 |
| CH-25 | 두 탭/보호자 분리 의견 → 네 탭 유지 | 공통 UX | 확정 |
| CH-26 | 단계 UI 통일 → 질문·입력·다음·음성 배치 | UX-003·011–012 | 명세 반영 |
| CH-27 | 복수 계좌·단일 확인 → 모두 지원 | FR-006·014·028 | 명세 반영 |
| CH-28 | 아들 송금 1→3 → 확인 유지·연속 순번 | FR-017 | 명세 반영 |
| CH-29 | 틀린 비밀번호 → 오류 문구·음성·입력 초기화 | FR-030·022 | 명세 반영 |
| CH-30 | 점선 추가 버튼 → 일반 버튼 | FR-006 | 명세 반영 |
| CH-31 | 기존 사람에게 계좌 추가 | FR-006 | 명세 반영 |
| CH-32 | FDS UI → 이유·대상·금액·결정, TTS 표식 제거 | FR-039, UX-012 | 명세 반영 |
| CH-33 | 로그인 후 계좌 불러오기 → 모의 후보·추가 화면 | FR-058 | 명세 반영 |
| CH-34 | 홈 작은 안내 제거 → 이용방법·도움말 | FR-060 | 명세 반영 |
...
- 고정 조건: 네 탭, 번호 확인창, 카톡 가족 알림 표시, 직접 입력 분리, 단일 계좌 확인, 금액 병기.
