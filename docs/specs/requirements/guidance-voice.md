...
| 뒤로·취소 | [편집 이탈 확인 모달](shared-ux.md#편집-이탈-확인)에서 계속 작성/버리기 선택, 다른 초안 보존 |
...
- 직접 송금의 기본 안내: 받는 사람은 ‘받는 사람 이름을 선택해 주세요.’, 받는 계좌는 ‘받는 계좌를 선택해 주세요.’. 송금 패턴에 저장된 맞춤 문구·가족 음성은 기존 선택 규칙을 따른다.

연금·관리비·공과금의 실행 후 결과는 [금융 조회의 결과 안내](financial-inquiries.md#연금관리비공과금-결과-안내)에 따라 자동 생성 문구·TTS를 사용한다. 실행 전 설명에는 기존 선택 규칙을 유지한다.
...
- 챗봇에서도 `1`, `1번`, `2번 실행해줘` 같은 1~12 번호 명령은 현재 활성 단축번호와 일치시켜 같은 기존 패턴 실행 흐름으로 연결한다.
...
| 홈 안내 | 듣는 중·확인 중·실패 안내는 확대된 단짝이 옆 말풍선에 표시. 버튼 아래 중복 안내 제거. 단축번호 위에 겹칠 수 있으나 하단 메뉴는 가리지 않음 |
| 말풍선 닫기 | X는 안내만 닫고 인식은 유지. 새 상태의 문구가 오면 다시 표시 |
