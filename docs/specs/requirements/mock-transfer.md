...
| 순서 | 화면 | 질문·행동 |
| --- | --- | --- |
| 1 | 보낼 계좌 선택 | ‘어느 계좌에서 보낼까요?’ → 계좌를 누르면 받는 사람 단계로 이동 |
| 2 | 받는 사람 | ‘받는 사람을 선택해 주세요.’ → 사람을 누르면 받는 계좌 단계로 이동 |
| 3 | 받는 계좌 | ‘받는 계좌를 선택해 주세요.’ → 계좌를 누르면 금액 단계로 이동 |
| 4 | 보낼 금액 | ‘얼마를 보낼까요?’ → 다음 |
...
- 직접 송금과 송금 패턴은 같은 단계 UI를 사용한다. 직접 송금의 받는 사람 단계에서 새 계좌 직접 입력도 선택할 수 있다.
- 받는 사람·계좌는 노란색 라디오로 선택 상태를 표시한다. 뒤로 이동 시 선택을 유지하고, 같은 사람을 다시 눌러도 받는 계좌를 유지한다.
- 기존 선택값만으로 자동 이동하지 않는다. 보낼 계좌를 누를 때 다음 단계로 이동한다.
